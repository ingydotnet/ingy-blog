...
There is a lot of misunderstanding in the wild about how YAML is B<loaded> from text to an in-memory data structure. It's understandable, because the L<YAML Spec|http://yaml.org/spec/1.2/spec.html> is a formidable beast to grok, and it concentrates heavily on the parsing rules, and less on the other phases.
...
So loading this as a C<5.2> is completely appropriate. In a multi-language environment like Travis-CI, users should quote things like this, thus indicating it should be a string:
